...
$custom_type = wpresidence_addon_get_post_type();

// Agents Post Type 		= estate_agent
// Properties Post Type 	= estate_property

switch( $custom_type ) {

	case "estate_agent":
		
		$wpresidence_addon->add_field( 'agent_position', 'Position', 'text' );

		$wpresidence_addon->add_field( 'agent_email', 'Email', 'text' );

		$wpresidence_addon->add_field( 'agent_phone', 'Phone', 'text' );

		$wpresidence_addon->add_field( 'agent_mobile', 'Mobile', 'text' );

		$wpresidence_addon->add_field( 'agent_skype', 'Skype', 'text' );

		$wpresidence_addon->add_field( 'agent_facebook', 'Facebook', 'text' );

		$wpresidence_addon->add_field( 'agent_twitter', 'Twitter', 'text' );

		$wpresidence_addon->add_field( 'agent_linkedin', 'LinkedIn', 'text' );

		$wpresidence_addon->add_field( 'agent_pinterest', 'Pinterest', 'text' );

		$wpresidence_addon->add_field( 'agent_instagram', 'Instagram', 'text' );

		$wpresidence_addon->add_field( 'agent_website', 'Website (without http)', 'text' );

		$wpresidence_addon->add_options( null, 'Appearance Options', array(
			...
			$wpresidence_addon->add_field( 'header_type', 'Header Type', 'radio', 	
				array(
					...
					'5' => array(
						'Google Map',
						$wpresidence_addon->add_field( 'page_custom_lat', 'Map - Center point Latitude', 'text' ),
						$wpresidence_addon->add_field( 'page_custom_long', 'Map - Center point Longitude', 'text' ),
						$wpresidence_addon->add_field( 'page_custom_zoom', 'Zoom Level for map (1-20)', 'text' ),
						$wpresidence_addon->add_field( 'min_height', 'Height of the map when closed', 'text', null, 'In pixels, example: 200' ),
						$wpresidence_addon->add_field( 'max_height', 'Height of map when open', 'text', null, 'In pixels, example: 600' ),
		...
		$wpresidence_addon->run(
			array(
				"post_types" => array( "estate_agent" )
			)
		);

		function wpresidence_addon_import( $post_id, $data, $import_options, $article ) {
			...
			foreach ( $fields as $field ) {
		        ...
		        if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( $field, $import_options ) ) {
		    ...
		    foreach ( $fields as $field ) {
	            ...
	            if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_image( $import_options ) ) {
	...
	case "estate_property":
		...
		$wpresidence_addon->add_field( 'property_label_before', 'Before Price Label', 'text', null, 'Example: Per Month' );
		...
		$wpresidence_addon->add_options( null, 'Advanced Options', array(
			...
			$wpresidence_addon->add_field( 'post_show_title', 'Show Title', 'radio',
				array(
					'yes' => 'Yes',
					'no' => 'No'
				) ),

			$wpresidence_addon->add_field( 'property_page_desing_local', 'Use a custom property page template', 'text', null, 'Must be template title, slug, or ID' ),
		...
		function wpresidence_addon_import( $post_id, $data, $import_options, $article ) {
		    ...
		    foreach ( $fields as $field ) {
		        ...
		        if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( $field, $import_options ) ) {
		            ...
		            if ( in_array( $field, $image_fields ) ) {
		            	...
		            	update_post_meta( $post_id, strtolower($key), $data[$field] );
		    ...
		    foreach ($fields as $field) {
		    	...
		    	if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( $field_, $import_options ) ) {
			...
			// find and assign custom page template
			$field = 'property_page_desing_local';

			if ( empty( $article['ID'] ) || $wpresidence_addon->can_update_meta( $field, $import_options ) ) {
					global $wpdb;

					if ( empty( $data[ $field ] ) ) {
						update_post_meta( $post_id, $field, '' );
					} elseif ( $found_posts = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . $wpdb->posts . " WHERE `post_title` = '%s' AND `post_type` = 'page' LIMIT 1;", $data[ $field ] ) ) ) {
						if ( get_post_meta( $found_posts->ID, '_wp_page_template', true ) == 'page_property_design.php' ) {
							update_post_meta( $post_id, $field, $found_posts->ID );
						}
					} elseif ( $found_posts = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . $wpdb->posts . " WHERE `post_name` = '%s' AND `post_type` = 'page' LIMIT 1;", $data[ $field ] ) ) ) {
							if ( get_post_meta( $found_posts->ID, '_wp_page_template', true ) == 'page_property_design.php' ) {
								update_post_meta( $post_id, $field, $found_posts->ID );
							}
					} elseif( $found_posts = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . $wpdb->posts . " WHERE `ID` = '%s' AND `post_type` = 'page' LIMIT 1;", $data[ $field ] ) ) ) {
							if ( get_post_meta( $found_posts->ID, '_wp_page_template', true ) == 'page_property_design.php' ) {
								update_post_meta( $post_id, $field, $found_posts->ID );
							}
					} else {
		    ...
		    if ( empty( $article['ID'] ) or ( $wpresidence_addon->can_update_meta( $field, $import_options ) && $wpresidence_addon->can_update_meta( 'property_latitude', $import_options ) && $wpresidence_addon->can_update_meta( 'property_longitude', $import_options ) && !empty ( $search ) ) ) {
		    ...
		    foreach ( $fields as $key => $value ) {
		        ...
		        if ( empty( $article['ID'] ) or $wpresidence_addon->can_update_meta( $key, $import_options ) ) {
...
function wpresidence_addon_get_post_type() {
	/**
